...
trendCharts = [];
  ngOnInit() {
    this.Service.getIntradayData('GOLD').subscribe(value =>{
    ...
    this.Service.getRecommendationTrend('GOLD').subscribe(trend =>{
      this.trends = trend[0];
      this.trendCharts = new Chart('trendChart',{
        type:'bar',
        data:{
          labels:["Barrick Gold Corporation"],
          datasets: [
            { 
    ...
    this.Service.getEarningsCalender('GOLD').subscribe(calender =>{
